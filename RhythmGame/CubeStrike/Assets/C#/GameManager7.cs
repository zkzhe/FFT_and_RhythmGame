...
public class GameManager7 : MonoBehaviour {
	...
	if(PlayerPrefs.GetInt("HighScore7")<PlayerPrefs.GetInt("Score7"))
	PlayerPrefs.SetInt("HighScore7",PlayerPrefs.GetInt("Score7"));
	PlayerPrefs.SetInt("Start",0);
	SceneManager.LoadScene(17);
	}
	...
	if(col.gameObject.tag=="Win")	//碰觸到Win標籤
		GetComponent<GameManager7>().Win();  //獲得GameManager內的Win函數
