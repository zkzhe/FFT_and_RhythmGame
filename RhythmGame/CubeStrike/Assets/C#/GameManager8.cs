...
public class GameManager8 : MonoBehaviour {
	...
	if(PlayerPrefs.GetInt("HighScore8")<PlayerPrefs.GetInt("Score8"))
	PlayerPrefs.SetInt("HighScore8",PlayerPrefs.GetInt("Score8"));
	PlayerPrefs.SetInt("Start",0);
	SceneManager.LoadScene(19);
	}
	...
	if(col.gameObject.tag=="Win")	//碰觸到Win標籤
		GetComponent<GameManager8>().Win();  //獲得GameManager內的Win函數
