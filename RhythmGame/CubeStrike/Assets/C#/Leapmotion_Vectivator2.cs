...
public class Leapmotion_Vectivator2 : MonoBehaviour {
	...
	void Start () {
	gm = GameObject.Find("GameManager2");	//抓取GameObject內的函數
	...
	void FixedUpdate () {
		...
		Instantiate(z,transform.position,Quaternion.identity);	//創造z粒子系統
			...
			gm.GetComponent<GameManager2>().AddStreak();		//抓取GameManager物件內的函數使用AddStreak()
			AddScore();
			aaa=false;
				  }   
	 ...
	 void AddScore(){
	 PlayerPrefs.SetInt("Score2",PlayerPrefs.GetInt("Score2")+gm.GetComponent<GameManager2>().GetScore());	//抓取Score.text後再讀取分數再用GameManager的函數加分
	 }
	...
	void OnTriggerEnter(Collider col){
        ...
        {
            gm.GetComponent<GameManager2>().RestStreak();
            note = col.gameObject;
            Destroy(note);
        }
