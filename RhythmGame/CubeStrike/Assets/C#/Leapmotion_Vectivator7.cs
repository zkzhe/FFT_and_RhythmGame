...
public class Leapmotion_Vectivator7 : MonoBehaviour {
	...
	void Start () {
	gm = GameObject.Find("GameManager7");	//抓取GameObject內的函數
	...
	void FixedUpdate () {
		...
		Instantiate(z,transform.position,Quaternion.identity);	//創造z粒子系統
			...
			gm.GetComponent<GameManager7>().AddStreak();		//抓取GameManager物件內的函數使用AddStreak()
			AddScore();
			aaa=false;
				  }   
	 ...
	 void AddScore(){
	 PlayerPrefs.SetInt("Score7",PlayerPrefs.GetInt("Score7")+gm.GetComponent<GameManager7>().GetScore());	//抓取Score.text後再讀取分數再用GameManager的函數加分
	 }
	...
	void OnTriggerEnter(Collider col){
        ...
        {
            gm.GetComponent<GameManager7>().RestStreak();
            note = col.gameObject;
            Destroy(note);
        }
