...
public class Leapmotion_Vectivator8 : MonoBehaviour {
	...
	void Start () {
	gm = GameObject.Find("GameManager8");   //抓取GameObject內的函數
	...
	void FixedUpdate () {
		...
		Instantiate(z,transform.position,Quaternion.identity);	//創造z粒子系統
			...
			gm.GetComponent<GameManager8>().AddStreak();		//抓取GameManager物件內的函數使用AddStreak()
			AddScore();
			aaa=false;
				  }   
	 ...
	 void AddScore(){
	 PlayerPrefs.SetInt("Score8",PlayerPrefs.GetInt("Score8")+gm.GetComponent<GameManager8>().GetScore());	//抓取Score.text後再讀取分數再用GameManager的函數加分
	 }
	...
	void OnTriggerEnter(Collider col){
        ...
        {
            gm.GetComponent<GameManager8>().RestStreak();
            note = col.gameObject;
            Destroy(note);
        }
