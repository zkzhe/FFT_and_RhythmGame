...
public class Leapmotion_Vectivator1 : MonoBehaviour {
	...
	void Start () {
	gm = GameObject.Find("GameManager1");	//抓取GameObject內的函數
	...
	void FixedUpdate () {
		...
		Instantiate(z,transform.position,Quaternion.identity);	//創造z粒子系統
			...
			gm.GetComponent<GameManager1>().AddStreak();		//抓取GameManager物件內的函數使用AddStreak()
			AddScore();
			aaa=false;
				  }   
	 ...
	 void AddScore(){
	 PlayerPrefs.SetInt("Score1",PlayerPrefs.GetInt("Score1")+gm.GetComponent<GameManager1>().GetScore());	//抓取Score.text後再讀取分數再用GameManager的函數加分
	 }
